...
FIG = "/home/fenghn/CvTest/experiments/figs"
...
m = YOLO(f"{RUNS}/yolov8m_B/weights/best.pt")
same_conf = {c: [] for c in range(NC)}
dec = {c: [0, 0, 0] for c in range(NC)}      # correct / confused / missed @0.25
for r in m.predict(source=ROOT+"/images/val", conf=0.001, iou=0.6, stream=True, verbose=False, device=2):
    H, W = r.orig_shape
